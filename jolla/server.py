...
static_setting = {"templates": r"templates"}
    ...
    def parse(self, urls):
        for url_handler in urls:
            ...
            if url_reg.match(self._path):
                ...
                return html_code
        raise HTTP404Error("REQUEST %s NOT FOUND IN ROUTE CONFIGURATION" % self._path)
...
class jolla_server(WSGIServer):
    def __init__(self, app, listener=None, log=None):
        ...
        if log:
            ...
            WSGIServer.__init__(
                self, listener=self.listener, application=self.application, log=logging
            )
        else:
            WSGIServer.__init__(
                self, listener=self.listener, application=self.application
            )
    ...
    def application(self, environ, start_response):
        ...
        header = [("Server", "Jolla/1.1")]

        for header_item in html_code[1]:
            header.append(header_item)
        start_response(status, header)

        return [html_code[0].encode()]

    def run_server(self):
        if self.host:
            print(
                "the jolla server is running on the {} in the port {}".format(
                    self.host, self.port
                )
            )
        else:
